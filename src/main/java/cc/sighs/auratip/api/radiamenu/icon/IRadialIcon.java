 ...
 * Custom implementations can be registered via {@link #register(ResourceLocation, Class, Codec)}.
 */
public interface IRadialIcon {
    ...
    Map<ResourceLocation, Codec<? extends IRadialIcon>> BY_ID = new ConcurrentHashMap<>();
    Map<Class<?>, ResourceLocation> ID_BY_CLASS = new ConcurrentHashMap<>();
     ...
     */
    static <T extends IRadialIcon> void register(ResourceLocation type, Class<T> clazz, Codec<T> codec) {
    ...
    default ResourceLocation type() {
        ResourceLocation id = ID_BY_CLASS.get(getClass());
    ...
    Codec<IRadialIcon> CODEC = new Codec<>() {
        ...
        public <T> DataResult<T> encode(IRadialIcon input, DynamicOps<T> ops, T prefix) {
            if (input instanceof TextureIcon(ResourceLocation location)) {
                return ResourceLocation.CODEC.encode(location, ops, prefix);
            }
