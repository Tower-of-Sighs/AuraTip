...
public class TipOverlay {
    ...
    public void render(GuiGraphics graphics, float partialTick, int mouseX, int mouseY, int screenWidth, int screenHeight) {
        ...
        if (tip.behavior().showPageIndicator() && tip.behavior().allowPaging() && pages.size() > 1) {
            String left = "<";
            String right = ">";
            graphics.drawString(Minecraft.getInstance().font, left, drawX + paddingL, indicatorY, 0xFFFFFFFF);
            graphics.drawString(Minecraft.getInstance().font, right, drawX + w - paddingR - Minecraft.getInstance().font.width(right), indicatorY, 0xFFFFFFFF);
        }
    ...
    private int drawTextElement(GuiGraphics graphics, ComponentSerialization.TextElement element, int x, int y) {
        ...
        int drawY = 0;
        for (var line : lines) {
            graphics.drawString(font, line, 0, drawY, 0xFFFFFFFF, true);
