    ...
    private static Action decode(Map<String, Dynamic<?>> raw) {
        ...
        if (customCodec != null) {
            ...
            return parsed.result().orElseThrow(() ->
                    new IllegalStateException("Failed to parse action '" + id + "': " + parsed.error().map(error -> error.message()).orElse("unknown error"))
            );
        }
    ...
    private static Map<String, Dynamic<?>> encode(Action action) {
        ...
        if (type != null) {
            ...
            JsonElement encoded = encodedResult.result().orElseThrow(() ->
                    new IllegalStateException("Failed to encode action '" + finalType + "': " + encodedResult.error().map(error -> error.message()).orElse("unknown error"))
            );
