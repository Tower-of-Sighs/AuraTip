...
function exportKubeJs() {
  ...
  if (mode === 'radial') {
    ...
    slots.forEach(s => {
      ...
      if (icon && typeof icon === 'object' && !Array.isArray(icon)) {
        ...
        const tmpl = (icon.template && typeof icon.template === 'object') ? icon.template
        ...
        // Build template JSON for ItemStackTemplateUtil.fromJson()
        ...
        push(`    new ItemIcon(ItemStackTemplateUtil.fromJson(${JSON.stringify(tmplJson)}), ${icoScale}),`);
    ...
    push('});');
    downloadText('auratip_radial_menu.nekojs.js', lines.join('\n') + '\n', 'text/javascript;charset=utf-8');
    return;
  }
  ...
  pages.forEach(p => {
    ...
    if (p.badge && typeof p.badge === "object") {
      ...
      if (bd.text && typeof bd.text === "object") {
        ...
        const __js = `    pg.badge(${compExpr(bd.text)}, ${JSON.stringify(String(bd.background_color || "#CC000000"))}, ${Number(bd.radius||4)}, ${JSON.stringify(posStr)});`;
        push(__js);
      }
    }
  ...
  push('});');
  downloadText('auratip_tip.nekojs.js', lines.join('\n') + '\n', 'text/javascript;charset=utf-8');
}
...
function exportJavaSnippet() {
  ...
  if (mode === 'radial') {
    ...
    push('import cc.sighs.auratip.util.ItemStackTemplateUtil;');
    ...
    slots.forEach(s => {
      ...
      if (iconRaw && typeof iconRaw === 'object' && !Array.isArray(iconRaw)) {
        const tmpl = (iconRaw.template && typeof iconRaw.template === 'object') ? iconRaw.template
        ...
        iconExpr = `new ItemIcon(ItemStackTemplateUtil.fromJson(${javaStr(tmplJson)}), ${icoScale}f)`;
      } else {
